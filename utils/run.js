...
registerFont('coursefont/OpenSauceSans-Regular.ttf', { family: 'Open Sauce Sans' });
...
async function makeCertificate(name, course) {
    ...
    // Load template image
    const template = await loadImage('cert.png');
    ...
    // Draw name
    ctx.fillText(name, (canvas.width - nameWidth) / 2, (canvas.height - FONT_SIZE) / 2 + 165);
    ...
    // Draw course
    ctx.fillText(course, (canvas.width - courseWidth) / 2, (canvas.height - COURSE_FONT_SIZE) / 2 + 220);

    // Load signature image
    const signature = await loadImage(SIGNATURE_PATH);

    // Draw signature
    ctx.drawImage(signature, (canvas.width - signature.width) / 2, canvas.height - signature.height - 115);
